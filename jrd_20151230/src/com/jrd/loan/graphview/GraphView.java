...
import com.jrd.loan.graphview.GraphViewStyle.GridStyle;
...
abstract public class GraphView extends LinearLayout {
    private final static String DATA = "9.999";
    ...
    private class GraphViewContentView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            // 默认画笔线的宽度
            paint.setStrokeWidth(DensityUtil.dip2px(context, 0.5f));
            // 取得x,y轴数值以及最大,最小,差值
            ...
            // x轴标签
            drawHorizontalLabels(canvas, border, horstart + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2), height, horlabels, graphwidth);
            ...
            for (int i = 0; i < graphSeries.size(); i++) {
                // 填充数值
                drawSeries(canvas, _values(i), graphwidth, graphheight, border, minX, minY, diffX, diffY, horstart + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2), graphSeries.get(i).style);
            }
    ...
    private class VerLabelsView extends View {
        ...
        public VerLabelsView(Context context) {
            super(context);
            setLayoutParams(new LayoutParams(getGraphViewStyle().getVerticalLabelsWidth() == 0 ? DensityUtil.dip2px(context, 30) : getGraphViewStyle().getVerticalLabelsWidth(), LayoutParams.MATCH_PARENT));
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            // 设置左侧刻度值宽度
            if (getGraphViewStyle().getVerticalLabelsWidth() == 0 && getLayoutParams().width != verLabelTextWidth + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2)) {
                setLayoutParams(new LayoutParams(getStrWidth(DATA), LayoutParams.MATCH_PARENT));
    ...
    private int getStrWidth(String str) {
        ...
        float scale = ScreenUtil.getScreenHeight(context) / 600f;
    ...
    protected String formatLabel(double value, boolean isValueX) {
        ...
        int i = isValueX ? 1 : 0;// true 是1 false 0

        if (numberformatter[i] == null) {
            numberformatter[i] = NumberFormat.getNumberInstance();

            numberformatter[i].setMinimumFractionDigits(3);
    ...
    /**
     * returns the maximal Y value of all data. warning: only override this, if you really know want
     * you're doing!
     */
    ...
    /**
     * returns the minimal Y value of all data. warning: only override this, if you really know want
     * you're doing!
     */
