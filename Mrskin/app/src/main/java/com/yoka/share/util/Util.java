...
public class Util
{
    ...
    public static byte[] WXBmpToByteArray(final Bitmap bmp, int w, int h,
            final boolean needRecycle) {
        int i;
        int j;
        int startX, startY;
        if (bmp.getHeight() > bmp.getWidth()) {
            ...
            j = bmp.getWidth();
            startX = 0;
            startY = (bmp.getHeight() - bmp.getWidth()) / 2;
        } else {
            i = bmp.getHeight();
            j = bmp.getHeight();
            startX = (bmp.getWidth() - bmp.getHeight()) / 2;
            startY = 0;
        }
        ...
        while (true) {
            localCanvas.drawBitmap(bmp, new Rect(startX, startY, bmp.getWidth()
                    - startX, bmp.getHeight() - startY), new Rect(0, 0, i, j),
                    null);
            Matrix matrix = new Matrix();
            matrix.postScale((float) w / (float) i, (float) h / (float) i); // 长和宽放大缩小的比例
            Bitmap resizeBmp = Bitmap.createBitmap(localBitmap, 0, 0,
                    localBitmap.getWidth(), localBitmap.getHeight(), matrix,
                    true);
            if (needRecycle)
                bmp.recycle();
            ByteArrayOutputStream localByteArrayOutputStream = new ByteArrayOutputStream();
            resizeBmp.compress(Bitmap.CompressFormat.JPEG, 100,
    ...
    public static Bitmap WBBmpToByteArray(final Bitmap bmp, int w, int h,
            final boolean needRecycle) {
        int i;
        int j;
        if (bmp.getHeight() > bmp.getWidth()) {
        ...
        // while (true) {
        localCanvas.drawBitmap(bmp, new Rect(0, 0, i, j), new Rect(0, 0, i, j),
                null);
