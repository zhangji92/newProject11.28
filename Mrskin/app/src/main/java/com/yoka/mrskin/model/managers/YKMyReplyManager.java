...
public class YKMyReplyManager extends YKManager {
	private static final String PATH = getBase()+ "my/commentreply";
	private static YKMyReplyManager singleton = null;
	private static Object lock = new Object();

	public static YKMyReplyManager getInstance() {
		synchronized (lock) {
			if (singleton == null) {
				singleton = new YKMyReplyManager();
			}
		}
		return singleton;
	}
	public YKHttpTask postYKMyReplyData(String page,String userId,final Callback callback) {
		...
		return super.postURL(PATH, parameters, new com.yoka.mrskin.model.managers.base.Callback() {
			...
			public void doCallback(YKHttpTask task, JSONObject object, YKResult result) {
				...
				if (result.isSucceeded()&&object!=null) {
					JSONArray tmpArray = object.optJSONArray("reply_list");
					if (tmpArray != null) {
