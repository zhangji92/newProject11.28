...
import com.yoka.mrskin.view.calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView
{
    ...
    public CalendarPickerView(Context context, AttributeSet attrs)
    {
        ...
        adapter = new MonthAdapter(context, attrs, this);
        a.recycle();
    ...
    public FluentInitializer init(Date minDate, Date maxDate, Locale locale) {
        ...
        SimpleDateFormat weekdayNameFormat = new SimpleDateFormat(getContext()
                .getString(R.string.day_name_format), locale);
        adapter.setWeekdayNameFormat(weekdayNameFormat);
        fullDateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, locale);
        ...
        CalendatUtil.setMidnight(minCal);
        CalendatUtil.setMidnight(maxCal);
        displayOnly = false;
    ...
    public class FluentInitializer
    {
        ...
        public FluentInitializer setShortWeekdays(String[] newShortWeekdays) {
            ...
            SimpleDateFormat weekdayNameFormat = new SimpleDateFormat(
                    getContext().getString(R.string.day_name_format), symbols);
            adapter.setWeekdayNameFormat(weekdayNameFormat);
            return this;
        }
        ...
        for (int c = 0; c < months.size(); c++) {
            ...
            if (selectedIndex == null) {
                for (Calendar selectedCal : selectedCals) {
                    if (CalendatUtil.sameMonth(selectedCal, month)) {
                        selectedIndex = c;
                        break;
                    }
                }
                if (selectedIndex == null && todayIndex == null
                        && CalendatUtil.sameMonth(today, month)) {
                    todayIndex = c;
                }
            }
        }
    ...
    public void setTitleTypeface(Typeface titleTypeface) {
        adapter.setTitleTypeface(titleTypeface);
        validateAndUpdate();
    }
    ...
    public void setDateTypeface(Typeface dateTypeface) {
        adapter.setDateTypeface(dateTypeface);
        validateAndUpdate();
    }
    ...
    public MonthView.Listener getMonthViewListener() {
        return listener;
    }

    public List<MonthDescriptor> getMonth() {
        return months;
    }

    public List<List<List<MonthCellDescriptor>>> getCell() {
        return cells;
    }
    ...
    private class CellClickedListener implements MonthView.Listener
    {
        ...
        public void handleClick(MonthCellDescriptor cell) {
            ...
            if (!CalendatUtil.betweenDates(clickedDate, minCal, maxCal)
    ...
    private void validateDate(Date date) {
        ...
        // + "%nminDate: %s%nmaxDate: %s%nselectedDate: %s", minCal.getTime(),
        // maxCal.getTime(),
        // date));
        // }
    }
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        CalendatUtil.setMidnight(newlySelectedCal);
    ...
    private Date applyMultiSelect(Date date, Calendar selectedCal) {
        ...
        for (Calendar cal : selectedCals) {
            if (CalendatUtil.sameDate(cal, selectedCal)) {
        ...
        for (List<List<MonthCellDescriptor>> monthCells : cells) {
            for (List<MonthCellDescriptor> weekCells : monthCells) {
                for (MonthCellDescriptor actCell : weekCells) {
                    ...
                    if (CalendatUtil.sameDate(actCal, searchCal)
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month,
        ...
        Calendar minSelectedCal = CalendatUtil.minDate(selectedCals);
        Calendar maxSelectedCal = CalendatUtil.maxDate(selectedCals);
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month
            ...
            for (int c = 0; c < 7; c++) {
                ...
                boolean isSelected = isCurrentMonth
                        && CalendatUtil.containsDate(selectedCals, cal);
                boolean isSelectable = isCurrentMonth
                        && CalendatUtil.betweenDates(cal, minCal, maxCal)
                        && isDateSelectable(date);
                boolean isToday = CalendatUtil
                        .sameDate(cal, adapter.getToday());
                boolean isHighlighted = CalendatUtil.containsDate(
                        highlightedCals, cal);
                int value = cal.get(DAY_OF_MONTH);

                MonthCellDescriptor.RangeState rangeState = MonthCellDescriptor.RangeState.NONE;
                if (selectedCals.size() > 1) {
                    if (CalendatUtil.sameDate(minSelectedCal, cal)) {
                        rangeState = MonthCellDescriptor.RangeState.FIRST;
                    } else if (CalendatUtil.sameDate(
                            CalendatUtil.maxDate(selectedCals), cal)) {
                        rangeState = MonthCellDescriptor.RangeState.LAST;
                    } else if (CalendatUtil.betweenDates(cal, minSelectedCal,
                            maxSelectedCal)) {
