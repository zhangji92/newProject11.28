 ...
 */
public class SelectImageDiskLruCache {
     ...
     * @return
     */
    public static SelectImageDiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        if (!cacheDir.exists()) {
            cacheDir.mkdir();
        }

        if (cacheDir.isDirectory() && cacheDir.canWrite()) {
            return new SelectImageDiskLruCache(cacheDir, maxByteSize);
        }
    ...
    /**
     * Constructor that should not be called directly, instead use {@link SelectImageDiskLruCache#openCache(Context, File, long)} which runs some extra checks before creating a DiskLruCache
     * instance.
     * 
     * @param cacheDir
     * @param maxByteSize
     */
    private SelectImageDiskLruCache(File cacheDir, long maxByteSize) {
        mCacheDir = cacheDir;
    ...
    public void clearCache() {
        SelectImageDiskLruCache.clearCache(mCacheDir);
    }
    ...
    /**
     * Removes all disk cache entries from the given directory. This should not be called directly, call {@link SelectImageDiskLruCache#clearCache(Context, String)} or
     * {@link SelectImageDiskLruCache#clearCache()} instead.
     * 
     * @param cacheDir
    ...
    /**
     * Writes a bitmap to a file. Call {@link SelectImageDiskLruCache#setCompressParams(CompressFormat, int)} first to set the target bitmap compression and format.
     * 
     * @param bitmap
     * @param file
     * @return
